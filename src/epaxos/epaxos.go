...
type LeaderBookkeeping struct {
	...
	//add by Hao
	//Queued    int64
	//PreAcceptStart int64
	//AcceptStart int64
	//Commit      int64
	//Executed    int64
}

func NewReplica(id int, thrifty bool, exec bool, dreply bool, beacon bool, durable bool, batch_size int) *Replica {
	N := config.GetConfig().N()
	r := &Replica{
		genericsmr.NewReplica(id, thrifty, exec, dreply),
	...
	for bf_PT = 1; math.Pow(2, float64(bf_PT))/float64(batch_size) < BF_M_N; {
		bf_PT++
	}
...
func (r *Replica) Run() {
	...
	dlog.Info("Waiting for client connections")
	...
	for !r.Shutdown {
		...
		case propose := <-onOffProposeChan:
			...
			propose.OutQueue = MakeTimestamp()
			propose.Slow = 0
...
func (r *Replica) updateConflicts(cmds []state.Command, replica int32, instance int32, seq int32) {
	for i := 0; i < len(cmds); i++ {
		//get does not generate conflict
...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	...
	if batchSize > int(r.maxBatchSize) {
		batchSize = int(r.maxBatchSize)
	}
	...
	kArray := make([]state.Key, batchSize)
	proposals := make([]*genericsmr.Propose, batchSize)
	cmds[0] = propose.Command
	proposals[0] = propose
	kArray[0] = propose.Command.K
	timeNow := MakeTimestamp()
	for i := 1; i < batchSize; i++ {
		prop := <-r.ProposeChan
		cmds[i] = prop.Command
		kArray[i] = prop.Command.K
		proposals[i] = prop
		prop.OutQueue = timeNow
	}

	// log.Printf("Server %d: KeyArray is %v\n", r.Id, kArray)

	//hack by Hao
	// for i := 0; i < batchSize; i++ {
	// 	r.ReplyProposeDetail(
	// 		&genericsmrproto.ProposeDetailReply{
	// 			OK:        TRUE,
	// 			CommandId: proposals[i].CommandId,
...
func (r *Replica) handlePreAcceptReply(pareply *epaxosproto.PreAcceptReply) {
	...
	if inst.lb.preAcceptOKs >= r.N/2 && inst.lb.allEqual && allCommitted && isInitialBallot(inst.ballot) {
		...
		if inst.lb.clientProposals != nil {
			timeNow := MakeTimestamp()
			inst.lb.clientProposals[0].Commit = timeNow
		}
...
func (r *Replica) handleAcceptReply(areply *epaxosproto.AcceptReply) {
	...
	if inst.lb.acceptOKs+1 > r.N/2 {
		...
		if inst.lb.clientProposals != nil {
			timeNow := MakeTimestamp()
			inst.lb.clientProposals[0].Commit = timeNow
			inst.lb.clientProposals[0].Slow = 1
		}
