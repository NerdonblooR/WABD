...
func NewReplica(id int, thrifty bool, exec bool, dreply bool) *Replica {
...
func (r *Replica) ConnectToPeers() {
	...
	for i := 0; i < int(r.Id); i++ {
		for done := false; !done; {
			ipAddr := config.GetConfig().Addrs[config.ID(fmt.Sprint(i))]
...
func (r *Replica) ReplyProposeDetail(reply *genericsmrproto.ProposeDetailReply, w *bufio.Writer) {
	//r.clientMutex.Lock()
	//defer r.clientMutex.Unlock()
	//w.WriteByte(genericsmrproto.PROPOSE_REPLY)
	reply.Marshal(w)
